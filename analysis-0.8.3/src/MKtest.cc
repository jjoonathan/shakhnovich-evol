...
boost::tuples::tuple<double,double,double> getPositions(PolySites::const_site_iterator beg,
						PolySites::const_site_iterator end,
						const unsigned &n,
						const vector<int> &intervals);
		  ...
		  if (nfixations == 1)
		      ...
		      else
			{
			  ...
			  //so we save the positions in the set "complex"
                boost::tuples::tuple<double,double,double> compPos =
						...
						const vector<int> &intervals)
{
  ...
  return boost::make_tuple(rv[0],rv[1],rv[2]);
}
